...
admin.html              隐藏管理页（邮箱验证码登录后增删改工程项目/展览/投影/成员，无导航入口）
css/
...
- 内容管理（`admin.html`）：`projects` / `exhibition_items` / `projection_files` 三张表匿名只读活跃项，写权限通过 `site_admins`（管理员邮箱白名单）+ `is_site_admin()` 函数（security definer）限定。管理员用邮箱验证码登录（OTP，同个人登录），邮箱手动输入、须在白名单内。
